...
def generate():
    ...
    for i in range(len(locations)):
        ...
        text = text + "\n" + cypher + " - " + locations[i][
            "name"] + " - $" + reduced_fare + " - " + percentage + "% off original fee ($" + str(original_fee) + ") - "
...
def new_location_dialog():
    ...
    tk.Label(master, text="Enter the new location's name:").grid(row=1, column=1)
    new_location_dialog_input = tk.Entry(master)
    new_location_dialog_input.grid(row=2, column=1)

    tk.Button(master, text="Add", command=new_location).grid(row=3, column=1)
